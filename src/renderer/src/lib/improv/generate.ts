...
export interface ImprovLlmConfigStatus {
  configured: boolean
...
export interface ImprovLlmConfigInput {
  apiKey: string
  model?: string
  baseUrl?: string
}
...
function readLlmEnvConfig(): LlmConfig | null {
...
function readLlmLocalConfig(): LlmConfig | null {
...
function readLlmConfig(): LlmConfig | null {
  return readLlmEnvConfig() || readLlmLocalConfig()
}

export function getImprovLlmConfigStatus(): ImprovLlmConfigStatus {
  ...
  const localConfig = readLlmLocalConfig()
  if (localConfig) {
    return {
      configured: true,
      source: 'localStorage',
...
export function saveImprovLlmConfig(config: ImprovLlmConfigInput): void {
  const normalized = {
    apiKey: config.apiKey.trim(),
    model: (config.model || 'gpt-4o-mini').trim(),
    baseUrl: (config.baseUrl || 'https://api.openai.com/v1').trim()
  }

  localStorage.setItem('improvTool.llmConfig', JSON.stringify(normalized))
}

export function clearImprovLlmConfig(): void {
  localStorage.removeItem('improvTool.llmConfig')
}
...
export async function generateImprovDetails(
  ...
  if (!config) {
    return fallbackGenerate(
      request,
      contextEntities,
      'No LLM config found. Using built-in fallback details (you can add a key in Improv settings or VITE_LLM_* env vars).'
    )
  }
