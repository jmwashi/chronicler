...
import {
  clearImprovLlmConfig,
  generateImprovDetails,
  getImprovLlmConfigStatus,
  saveImprovLlmConfig
} from '@/lib/improv/generate'
...
export function ImprovToolTab({ session, activeScene }: Props): ReactElement {
  ...
  const [selectedRefKeys, setSelectedRefKeys] = useState<string[] | null>(null)
  const [result, setResult] = useState<Awaited<ReturnType<typeof generateImprovDetails>> | null>(
    null
  )
  const [isGenerating, setIsGenerating] = useState(false)
  const [showLlmSettings, setShowLlmSettings] = useState(false)
  const [llmStatus, setLlmStatus] = useState(() => getImprovLlmConfigStatus())
  const [llmForm, setLlmForm] = useState({
    apiKey: '',
    model: llmStatus.model || 'gpt-4o-mini',
    baseUrl: llmStatus.baseUrl || 'https://api.openai.com/v1'
  })
  ...
  const effectiveSelectedRefKeys = selectedRefKeys ?? defaultRefKeys

  const contextRefs: ImprovEntityRef[] = effectiveSelectedRefKeys.map((value) => {
  ...
  const toggleRef = (ref: ImprovEntityRef): void => {
    ...
    setSelectedRefKeys((current) => {
      const base = current ?? defaultRefKeys
      return base.includes(key) ? base.filter((entry) => entry !== key) : [...base, key]
    })
  }

  const refreshLlmStatus = (): void => setLlmStatus(getImprovLlmConfigStatus())

  const saveLlmSettings = (): void => {
    if (!llmForm.apiKey.trim()) return
    saveImprovLlmConfig(llmForm)
    setLlmForm((current) => ({ ...current, apiKey: '' }))
    refreshLlmStatus()
  }

  const clearLlmSettings = (): void => {
    clearImprovLlmConfig()
    refreshLlmStatus()
  }
    ...
    <div className="space-y-4">
      <div className="rounded-lg border p-3" style={{ borderColor: 'hsl(var(--border))' }}>
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-sm font-semibold">LLM setup (optional)</h3>
          <button
            onClick={() => setShowLlmSettings((current) => !current)}
            className="text-xs underline"
            style={{ color: 'hsl(var(--primary))' }}
          >
            {showLlmSettings ? 'Hide setup' : 'Configure'}
          </button>
        </div>
        <p className="text-xs mt-1" style={{ color: 'hsl(var(--muted-foreground))' }}>
          Status:{' '}
          {llmStatus.configured
            ? `Configured via ${llmStatus.source}`
            : 'Not configured (fallback-only mode)'}
        </p>
          ...
          <div className="grid md:grid-cols-3 gap-3 mt-3">
            <label>
              ...
              <input
                ...
                onChange={(e) => setLlmForm((current) => ({ ...current, apiKey: e.target.value }))}
                placeholder="sk-..."
            ...
            <label>
              ...
              <input
                value={llmForm.model}
                onChange={(e) => setLlmForm((current) => ({ ...current, model: e.target.value }))}
                className="w-full px-2.5 py-2 rounded-md text-sm border"
                style={{
                  borderColor: 'hsl(var(--border))',
                  backgroundColor: 'hsl(var(--background))'
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) => setLlmForm((current) => ({ ...current, baseUrl: e.target.value }))}
                className="w-full px-2.5 py-2 rounded-md text-sm border"
                style={{
                  borderColor: 'hsl(var(--border))',
                  backgroundColor: 'hsl(var(--background))'
      ...
      <div className="rounded-lg border p-3" style={{ borderColor: 'hsl(var(--border))' }}>
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            onClick={() => setSelectedRefKeys(null)}
            className="text-xs underline"
            style={{ color: 'hsl(var(--primary))' }}
          >
                  ...
                  groupedOptions[kind].map((option) => {
                    ...
                    return (
                      <label key={key} className="flex items-center gap-2 text-xs">
                        <input
                          type="checkbox"
                          checked={effectiveSelectedRefKeys.includes(key)}
