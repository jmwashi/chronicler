...
export default function LorePage() {
  ...
  const { lore, add, update, remove } = useLoreStore()
  const characters = useCharacterStore((s) => s.characters)
  const [modalOpen, setModalOpen] = useState(false)
  const [editing, setEditing] = useState<Lore | undefined>()
  ...
  const filtered = lore.filter((l) => l.campaignId === activeCampaignId)
  ...
  const openEdit = (l: Lore) => { setEditing(l); setModalOpen(true) }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: 'hsl(var(--foreground))' }}>Lore</h1>
          <p className="text-sm mt-0.5" style={{ color: 'hsl(var(--muted-foreground))' }}>World history, legends, and knowledge</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <EmptyList message="No lore yet — add some world-building to get started." />
      ) : (
        <>
          <div className="flex items-center gap-2 mb-4">
            <div className="relative flex-1">
              ...
              <input className="w-full pl-8 pr-3 py-1.5 rounded-md text-sm border bg-transparent outline-none"
                ...
                placeholder="Search lore..." value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
          ...
          {displayed.length === 0 ? (
            <EmptyList message={`No lore matches "${search}"`} />
          ) : (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
              {displayed.map((l) => (
                <div key={l.id}
                  ...
                  onClick={() => { incrementNavDepth(); navigate(`/lore/${l.id}`) }}>
                  ...
                  <div className="pr-10">
                    <h3 className="font-semibold text-sm truncate mb-2" style={{ color: 'hsl(var(--foreground))' }}>{l.title}</h3>
                    <div className="flex flex-wrap gap-1">
                      <Badge>{l.category}</Badge>
                      {l.era && <Badge>{l.era}</Badge>}
                    </div>
                    {l.description && (
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title={editing ? 'Edit Lore' : 'New Lore'}>
        <LoreForm key={editing?.id ?? 'new'} lore={editing} campaignId={activeCampaignId} characters={campaignChars} onSave={handleSave} onCancel={closeModal} />
      </Modal>
    </div>
  )
}
